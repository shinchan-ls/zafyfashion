// app/page.tsx
// FIXES:
//   ABORT-SAFE — signal?.aborted prevents StrictMode double-invoke
//                from flashing "0 products" on first load
//   DEDUP      — seenIds ref prevents duplicate cards on scroll
...
export default function Home() {
  ...
  const fetchPage = useCallback(async (pageNum: number, signal?: AbortSignal) => {
    ...
    try {
      ...
      // API default route returns a plain array
      ...
      // ✅ KEY FIX: In React StrictMode (dev) effects run twice.
      // The first run's AbortController fires before the fetch completes → sets
      // initialLoading=false with products=[] → flashes empty state.
      // Checking signal?.aborted skips the state update for cancelled requests,
      // keeping the skeleton visible until the real (second) fetch succeeds.
      if (!signal?.aborted) {
  ...
  return (
    <div className="bg-white min-h-screen flex flex-col">
      ...
      {/* Trending Products — infinite scroll */}
      <section className="py-16 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="flex justify-between items-end mb-10">
            <h2 className="text-3xl md:text-4xl font-light tracking-wide">Trending Products</h2>
            <Link href="/products" className="text-black hover:underline font-medium text-sm">
              View All →
            </Link>
          </div>
